...
fn main() {
    ...
    //     file.write_fmt(format_args!("(exit_after: Some({frames}))")).unwrap();
    ...
    //     cmd!(
    //         sh,
    //         "cargo build {ci_testing...} --release --target wasm32-unknown-unknown --example
    // {example}"     )
    //     .run()
    //     .expect("Error building example");
    //     cmd!(
    //         sh,
    //         "wasm-bindgen --out-dir examples/wasm/target --out-name wasm_example --target
