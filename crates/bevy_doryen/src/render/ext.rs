...
/// Adds methods to the [`App`] for adding systems to the Doryen
/// [`render`](crate::doryen::Engine::render) schedule.
pub trait RenderSystemExt {
    /// Adds a system to the [`DoryenRenderStage::Render`] stage of the
    /// render schedule.
    fn add_doryen_render_system<Params>(
        &mut self,
    ...
    /// Adds a system set to the [`DoryenRenderStage::Render`] stage of the
    /// render schedule.
    ...
    /// already added the State to the main Bevy app
    ///
    ...
    /// [`Stage`] in Bevy will have the same issue.
    ///
...
impl RenderSystemExt for App {
    ...
    where
        T: Component + Debug + Clone + Eq + Hash,
    {
        self.add_doryen_render_system_set_to_stage(
